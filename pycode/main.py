...
def interProvinModel(vre_year, res_tag, init_data, is8760, curr_year, scen_params):
    ...
    # Firm resource capacity limit by province (nuclear, coal, hydro, gas, beccs)
    ...
    for pro in provins:
        cap_gw[pro] = {}
        cap_hourly[pro] = {}
        for tech in ["coal_unabated", "coal_ccs", "chp_ccs", "hydro", "nuclear", "beccs", "gas_unabated", "gas_ccs"]:
        ...
        if not scen_params["scenario"]["endogenize_firm_capacity"]:
            for tech in ["coal_unabated", "coal_ccs", "chp_ccs",
                         "hydro", "nuclear", "beccs", "gas_unabated", "gas_ccs"]:
    ...
    coal_ccs_cost_fixed_per_hour = \
        ((coal_capex + coal_ccs_capex) * getCRF(weighted_average_cost_of_capital, coal_lifespan) + coal_ccs_opex_fixed) / 8760
    ...
    gas_ccs_cost_fixed_per_hour = \
        ((gas_capex + gas_ccs_capex) * getCRF(weighted_average_cost_of_capital, gas_lifespan) + gas_ccs_opex_fixed) / 8760
    ...
    beccs_cost_fixed = [beccs_cost_fixed_per_hour *
                        cap_gw[pro]["beccs"] *
                        scen_params["optimization_hours"]["days"] * 24 for pro in provins]
    beccs_cost_var_must_run = [scen_params['beccs']['var_cost'] *
                               cap_gw[pro]["beccs"] * scen_params['beccs']['must_run'] *
                               scen_params["optimization_hours"]["days"] * 24 for pro in provins]
    ...
    interProvinModel.setObjective(gp.quicksum(wind_gen_cost) +
                                  ...
                                  gp.quicksum(beccs_cost_var_must_run) +
    ...
    # Consider medium-term policy goal for VRE buildout
    ...
    # BECCS + Gas Unabated/CCS + CHP_CCS + Coal Unabated/CCS emissions <= annual emission targets
    emission_scenario = scen_params["scenario"]["emission_target"]
    emission_factor_method = scen_params["scenario"]["emission_factor_method"]
    emission_targets = pd.read_csv(
        os.path.join(work_dir, "data_csv", "capacity_assumptions", f"power_sector_emission_{emission_scenario}.csv"))
    ...
    f_objV.write('%s,%s,%s\n' % ("beccs_cost_var_must_run", np.round(gp.quicksum(beccs_cost_var_must_run).getValue(), 2), "Const"))
    ...
    emission_breakdown = pd.DataFrame(
        columns=["province", "cap_coal_unabated_gw", "cap_coal_ccs_gw", "cap_chp_ccs_gw",
                 "cap_beccs_gw", "cap_hydro_gw", "cap_nuclear_gw", "cap_gas_unabated_gw", "cap_gas_ccs_gw",
                 ...
                 "emission_beccs_mt", "emission_coal_unabated_mt", "emission_coal_ccs_mt", "emission_chp_ccs_mt",
