...
def initCellData(vre: str, vre_year_single: str, hour_seed: list, res_tag: str, vre_year: str,
    ...
    # Specify the remaining magic numbers
    r = 0.062  # TODO these are used for calculating VRE electricity prices
    N = 25
    iN = 15
    dN = 15
    ...
    # Specify 2015/2016 cell/mat file names
    cell_file = {
        'wind': {
            '2016': {
    ...
    if vre == 'wind':
        for file in cell_file[vre][vre_year_single]:
            for pro in provins:
                provin_cell[pro] = {}
                provin_cell_lon[pro] = {}
                provin_cell_lat[pro] = {}
            ...
            # mat files has format of id_latId_lonId
            # mat files contains generation capacity at each hour of the year 
            # at each cell
            for root, dirs, files in os.walk(work_dir + 'data_mat' + dir_flag + mat_file[vre][vre_year_single][file]):
                for mat in files:
                    ...
                    for pro in provins:
                        if (cell_flag[1], cell_flag[2]) in provin_cell[pro]:
                            ...
                            cell_cf_prof = scio.loadmat(
                                work_dir + 'data_mat' + dir_flag + mat_file[vre][vre_year_single][file] +
                                dir_flag + mat)['X_cf']
                            ...
                            if CF >= 0.0020 and gen_cost >= 0 and cell_gen_poten != 0:
                                ...
                                # what do 0.15625 and 0.125 mean? 
        ...
        vre_coordinate = {}
        f_coor = open(
            work_dir + 'data_csv' + dir_flag + "vre_installations" + dir_flag +
            'inter_connect_China_solarpower_coordinate_' + vre_year_single + '.csv',
            'r+')
        for line in f_coor:
            line = line.replace('\n', '')
            line = line.split(',')
        ...
        for i in vre_cell['index']:
            ...
            if CF >= 0.0020 and gen_cost >= 0 and cap_poten > 0:
                ...
                # TODO does the following equation make sense?
...
def initDemLayer(vre_year: str, res_tag: str, curr_year: int, scen_params: dict):
    ...
    for pro in provin_py:
        ...
        pro_layer_cf[pro]["beccs_hourly_cf"] = pro_layer_cf[pro]["bio_hourly_cf"]
    ...
    for pro in provin_py:
        ...
        pro_layer_cap[pro]["gas_ccs"] = np.round(gasBeta * pro_layer_cf[pro]["gas_ccs_hourly_cf"] *
                                                 pro_layer_cap_gw[pro]["gas"], 2)
        pro_layer_cap[pro]["beccs"] = np.round(gasBeta * pro_layer_cf[pro]["bio_hourly_cf"] *
